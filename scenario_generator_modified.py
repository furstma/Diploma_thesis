...
def get_moments_of_samples(sampled_outcomes, N_STOCKS,N_OUTCOMES):
    ...
    mean = np.sum(sampled_outcomes, axis=1) / N_OUTCOMES
    calculated_moments = np.column_stack((calculated_moments, mean))
    for i in np.arange(2, 13):
        ith_moment = np.sum(np.power(sampled_outcomes -np.reshape(mean, (N_STOCKS,1)), i), axis=1) / N_OUTCOMES
...
# -----OUTPUT PHASE-----
#   3.5) check and repare TRSFMOM
TRSFMOM = np.column_stack((TRSFMOM[:,0],
                           TRSFMOM[:,1],
                           np.clip(TRSFMOM[:,2], -3,3),
                           np.clip(TRSFMOM[:,3], 0.5,10)))


#   4) generate outcome of X_i for i=1,...,n
#       -sample from N(0,1)
#       -use cubic transformation to match the moments
#      iterate till the correlation matrix of X is close to I_n
...
Rp = np.corrcoef(X)
p=1
I = np.diag(np.ones(N_STOCKS))
def rmse(a,b):
    return np.sqrt(np.mean((a - b) ** 2))

Lp = np.linalg.cholesky(Rp)
X_backward = np.linalg.inv(np.linalg.cholesky(Rp))*X

max_error =
while rmse(Rp, I) > max_error:
